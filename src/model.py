...
class ConvMixer(nn.Module):
    def __init__(self):
        ...
        # MLP applied along the feature dimension
        self.conv = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=(2, 3), padding=(0, 1))
        self.act = nn.GELU()
    ...
    def forward(self, x):
        ...
        #x =torch.concatenate([x1,x4],dim=1)
        #x = self.dropout(x)
        x = torch.squeeze(x,2)
        x= x.permute(0,2,1)
        x = self.lin(x)
...
class CLIPVAD(nn.Module):
    ...
    def encode_video(self, images, padding_mask, lengths):
        ...
        disadj = self.disAdj(x.shape[0], x.shape[1]).to(self.device)
    ...
    def encode_textprompt(self, text):
        word_tokens = clip.tokenize(text,context_length=77).to(self.device)
    ...
    def forward(self, visual, padding_mask, text, lengths,T_feats,label_clip_feats):
        ...
        t_feat_attn = attention_weights@((T_feats).transpose(-1, -2))
        t_feat_attn = self.layer_norm(T_feats+t_feat_attn.transpose(-1, -2))
        t_feat_attnx = self.act(self.fc1(t_feat_attn))
        ...
        return visual_features,label_clip_feats/label_clip_feats.norm(dim=-1, keepdim=True),T_feats/T_feats.norm(dim=-1, keepdim=True),text_features_ori, logits1, logits2
